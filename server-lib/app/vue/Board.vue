...
function getImageName (cp) {
  ...
  let num
  if (color === 'Black') {
    num = pieceToNum[piece]
  } else if (color === 'White') {
    num = pieceToNum[piece] + 30
  }
...
module.exports = {
  directives: {
    showBoard: function (canvas, data) {
      ...
      const stripeImg = document.getElementById('sprite')

      const board = sprite.board.frame
      ctx.drawImage(stripeImg, board.x, board.y, board.w, board.h, 0, 0, board.w, board.h)

      for (let j = 0; j < 9; j++) {
        for (let i = 0; i < 9; i++) {
          const cp = data.value[j][8 - i]
          if (cp) {
            const name = getImageName(cp)
            const frame = sprite[name].frame
            ctx.drawImage(stripeImg, frame.x, frame.y, frame.w, frame.h, 30 + 60 * i, 30 + 64 * j, frame.w, frame.h)
          }
        }
      }
    }
